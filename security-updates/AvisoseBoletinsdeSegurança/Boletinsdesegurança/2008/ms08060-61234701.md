...
ms:assetid: 'ms08-060'
ms:contentKeyID: 61234701
ms:mtpsurl: 'https://technet.microsoft.com/pt-PT/library/ms08-060(v=Security.10)'
---
...
Vulnerabilidade no Active Directory Poderia Permitir a Execução Remota de Código (957280)
-----------------------------------------------------------------------------------------
...
Esta actualização de segurança resolve uma vulnerabilidade comunicada de forma privada sobre implementações do Active Directory no Microsoft Windows 2000 Server. A vulnerabilidade poderia permitir a execução remota de código se um intruso obtivesse acesso a uma rede afectada. Esta vulnerabilidade apenas afecta os servidores com o Microsoft Windows 2000 configurados como controladores de domínio. Se um servidor com o Microsoft Windows 2000 não tiver sido promovido a controlador de domínio, não serão atendidos pedidos do Lightweight Directory Access Protocol (LDAP) ou do LDAP através de SSL (LDAPS), não sendo exposto a esta vulnerabilidade.

Esta actualização de segurança está classificada como Crítica para implementações do Active Directory no Microsoft Windows 2000 Server. Para mais informações consulte a subsecção **Software Afectado e Software Não Afectado**, nesta secção.

A actualização de segurança resolve a vulnerabilidade corrigindo a atribuição de memória para os pedidos de cliente LDAP. Para mais informações sobre a vulnerabilidade, consulte a subsecção de Perguntas Mais Frequentes (FAQ) quanto à vulnerabilidade específica na secção seguinte, **Informações sobre as vulnerabilidades**.
...
|----------------------------------------------|-------------------------------------------------------------------------------------------------------------------|-----------------------------|-------------------------------------|-----------------------------------------------------------|
| Microsoft Windows 2000 Server Service Pack 4 | [Active Directory](http://www.microsoft.com/downloads/details.aspx?familyid=8ed7bb9a-4b26-49d7-8c14-60226d2bc20d) | Execução Remota de Código   | Crítica                             | [MS08-035](http://go.microsoft.com/fwlink/?linkid=111953) |
...
Os pormenores sobre as informações do ficheiro podem ser encontrados no [Artigo 957280 da Base de Dados de Conhecimento da Microsoft](http://support.microsoft.com/kb/957280).
...
A actualização só será oferecida para sistemas onde o componente afectado esteja instalado e activado. Os sistemas Microsoft Windows 2000 Server que não desempenham o papel de controlador de domínio não são afectados e não receberão esta actualização.
  
**Esta vulnerabilidade também afecta o Active Directory Application Mode (ADAM) ou o Active Directory Lightweight Directory Service (AD LDS)?**    
Não. Esta vulnerabilidade apenas afecta os servidores com o Microsoft Windows 2000 configurados como controladores de domínio Active Directory. As outras plataformas ou serviços não são afectados.
...
<th style="border:1px solid black;" >Vulnerabilidade de Sobrecarga no Active Directory - CVE-2008-4023</th>
...
Vulnerabilidade de Sobrecarga no Active Directory - CVE-2008-4023  
-----------------------------------------------------------------
  
<span></span>
Existe uma vulnerabilidade de execução remota de código em implementações do Active Directory no Microsoft Windows 2000 Server. A vulnerabilidade deve-se à atribuição incorrecta de memória durante a recepção de pedidos do LDAP ou do LDAPS especialmente concebidos para o efeito. Um intruso que conseguisse tirar partido desta vulnerabilidade poderia obter o controlo total de um sistema afectado.
  
Para visualizar esta vulnerabilidade como uma entrada comum na lista de Vulnerabilidades e Exposições Comuns, consulte o boletim [CVE-2008-4023](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2008-4023).
  
#### Factores Atenuantes para a Vulnerabilidade de Sobrecarga no Active Directory - CVE-2008-4023
...
-   Esta vulnerabilidade apenas afecta os servidores com o Microsoft Windows 2000 configurados como controladores de domínio. Se um servidor com o Microsoft Windows 2000 não tiver sido promovido a controlador de domínio, não serão atendidos pedidos do LDAP ou do LDAPS, não sendo exposto a esta vulnerabilidade.  
    Para confirmar se um servidor atende a pedidos do LDAP ou do LDAPS, execute o seguinte comando a partir de uma linha de comandos com direitos de administrador e verifique se os pedidos estão a ser processados na porta do LDAP (389) ou na porta do LDAPS (636):
  
    `netstat –a`
  
    O LDAP está activado se os resultados contiverem as seguintes informações:
  
    `Proto Local Address Foreign Address StateTCP 0.0.0.0:389 0.0.0.0:0 LISTENINGTCP 0.0.0.0:636 0.0.0.0:0 LISTENING`
  
#### Soluções Alternativas para a Vulnerabilidade de Sobrecarga no Active Directory - CVE-2008-4023
  
Uma solução alternativa refere-se a uma definição ou alteração de configuração que não corrige a vulnerabilidade subjacente, mas que ajudaria a bloquear vectores de ataque conhecidos antes de aplicar a actualização. A Microsoft testou as seguintes soluções alternativas e estados na discussão acerca de casos em que uma solução alternativa reduz a funcionalidade:
  
-   **Bloqueio das portas TCP 389 e 636 na firewall de perímetro**  
    Estas portas são utilizadas para iniciar uma ligação com o componente afectado. O bloqueio destas portas na firewall da empresa, a nível de recepção e envio, ajuda a impedir que os sistemas protegidos por essa firewall sejam atacados por tentativas de explorar esta vulnerabilidade. Recomendamos que bloqueie todas as comunicações de entrada não solicitadas a partir da Internet, para ajudar a impedir ataques que possam utilizar outras portas. Para mais informações sobre portas, consulte [Atribuição de portas TCP e UDP](http://go.microsoft.com/fwlink/?linkid=21312). Para mais informações sobre a Firewall do Windows, consulte [Como configurar a Firewall do Windows num Único Computador](http://www.microsoft.com/technet/security/smallbusiness/prodtech/windowsxp/cfgfwall.mspx).
  
#### Perguntas Mais Frequentes sobre a Vulnerabilidade de Sobrecarga no Active Directory - CVE-2008-4023
...
**Nota.** As tentativas de exploração desta vulnerabilidade poderiam também resultar numa condição de negação de serviço. Um intruso poderia também explorar esta vulnerabilidade para forçar o sistema a reiniciar. Contudo, uma vez que a execução remota de código é possível, a abrangência agregada é superior à de uma negação de serviço.
  
**O que provoca a vulnerabilidade?**    
O serviço LDAP atribui incorrectamente memória a pedidos LDAP especialmente concebidos para o efeito.
...
**O que é o LDAP?**    
O Lightweight Directory Access Protocol ([LDAP](http://msdn2.microsoft.com/en-us/library/aa503238.aspx)) é um padrão de protocolos de rede aberto concebido para fornecer acesso a directórios distribuídos.
  
**O que é o LDAP através de SSL?**    
Por predefinição, o tráfego LDAP é transmitido de forma não segura. Contudo, é possível tornar confidencial o tráfego LDAP e protegê-lo através da utilização da tecnologia Secure Sockets Layer (SSL) / Transport Layer Security (TLS). Os administradores podem activar o LDAP através de SSL (LDAPS) instalando um certificado devidamente formatado de uma autoridade com certificação da Microsoft (CA) ou de uma entidade CA que não seja da Microsoft, de acordo com as orientações no [Artigo 321051 da Base de Dados de Conhecimento da Microsoft](http://support.microsoft.com/kb/321051/en-us).
...
Um intruso poderia tentar explorar a vulnerabilidade enviando um pacote LDAP especialmente concebido para o efeito para um Controlador do Domínio do Microsoft Windows 2000. Qualquer utilizador anónimo com acesso à rede visada poderia entregar um pacote de rede especialmente concebido para o efeito ao sistema afectado de forma a explorar esta vulnerabilidade.
  
**Quais os sistemas mais susceptíveis face a esta vulnerabilidade?**    
O LDAP é um requisito para o Active Directory. Qualquer Windows 2000 Server que esteja a ser executado como um servidor Active Directory é afectado por esta vulnerabilidade.
  
**O que faz a actualização?**    
A actualização resolve a vulnerabilidade corrigindo a atribuição de memória para os pedidos de cliente LDAP especialmente concebidos para o efeito.
...
<td style="border:1px solid black;">Microsoft Windows 2000 Service Pack 4:<br />
Utilize a ferramenta Adicionar ou Remover Programas no Painel de Controlo ou o utilitário Spuninst.exe que está localizado na pasta %Windir%\$NTUninstallKB957280$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Informações de Ficheiros</strong></td>
<td style="border:1px solid black;">Consulte o <a href="http://support.microsoft.com/kb/957280">Artigo 957280 da Base de Dados de Conhecimento da Microsoft</a></td>
</tr>
<tr class="even">
...
-   Paul Miseiko, da [nCircle](http://www.ncircle.com/), por fornecer informações sobre a Vulnerabilidade de Sobrecarga no Active Directory (CVE-2008-4023)
...
-   V1.1 (15 de Outubro de 2008): Actualização da tabela de Software Não Afectado.
