---
TOCTitle: 'MS10-095'
Title: 'Microsoft Security Bulletin MS10-095 - Importante'
ms:assetid: 'ms10-095'
ms:contentKeyID: 61234970
ms:mtpsurl: 'https://technet.microsoft.com/pt-PT/library/ms10-095(v=Security.10)'
---
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------|-------------------------------------|---------------------------------------------|
| [Windows 7 para sistemas de 32 bits](http://www.microsoft.com/downloads/details.aspx?familyid=75591d37-2cb8-4cdf-acbb-89cd0d1a9290)                       | Execução Remota de Código   | Importante                          | Nenhum                                      |
| [Windows 7 para sistemas baseados em x64](http://www.microsoft.com/downloads/details.aspx?familyid=3a0c4dd0-98b4-4e7a-99ed-22b9d9f76cd1)                  | Execução Remota de Código   | Importante                          | Nenhum                                      |
| [Windows Server 2008 R2 para sistemas baseados em x64](http://www.microsoft.com/downloads/details.aspx?familyid=f58a765f-cea9-456d-b0ab-bfc70b109cbf)\*   | Execução Remota de Código   | Importante                          | Nenhum                                      |
| [Windows Server 2008 R2 para sistemas baseados em Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=13a9f838-ac07-43dc-9aee-a77207998e1e) | Execução Remota de Código   | Importante                          | Nenhum                                      |
...
<th style="border:1px solid black;" >Vulnerabilidade no Carregamento de Biblioteca BranchCache Não Segura - CVE-2010-3966</th>
...
Vulnerabilidade no Carregamento de Biblioteca BranchCache Não Segura - CVE-2010-3966  
------------------------------------------------------------------------------------
  
<span></span>
Há uma vulnerabilidade na execução remota de códigos na forma como o Microsoft Windows abre ficheiros específicos em plataformas que não suportam a funcionalidade de BranchCache. Um intruso que conseguisse tirar partido desta vulnerabilidade poderia obter o controlo total de um sistema afectado. Um intruso poderia então instalar programas; ver, alterar ou eliminar dados; ou ainda criar novas contas com todos os privilégios. Os utilizadores cujas contas estão configuradas com menos direitos de utilização no sistema podem correr menos riscos do que os utilizadores que trabalham com direitos de utilização de administrador.
  
Para visualizar esta vulnerabilidade como uma entrada comum na lista de Vulnerabilidades e Exposições Comuns, consulte [CVE-2010-3966](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-3966).
  
#### Factores Atenuantes para a Vulnerabilidade no Carregamento de Biblioteca BranchCache Não Segura - CVE-2010-3966
...
#### Soluções Alternativas para a Vulnerabilidade no Carregamento de Biblioteca BranchCache Não Segura - CVE-2010-3966
...
#### Perguntas mais frequentes sobre a Vulnerabilidade no Carregamento de Biblioteca BranchCache Não Segura - CVE-2010-3966
...
Esta vulnerabilidade é causada quando o Windows tenta validar se um ficheiro DLL específico utilizado pelo Windows BranchCache existe no sistema. Quando o DLL predefinido existir no sistema, este binário será procurado primeiro em localizações seguras locais, na estrutura de pastas do Windows. No entanto, quando não for encontrado, a aplicação tentará encontrar o ficheiro na localização onde o ficheiro é aberto, que pode ser uma localização não segura, como por exemplo uma partilha de rede. Um ficheiro especialmente concebido para o efeito, colocado numa localização alternativa, pode fazer com que o Windows execute o código escolhido pelo intruso no contexto do utilizador com sessão iniciada.
  
A biblioteca em questão carregada pelo Windows BranchCache só está presente nas edições do Windows que são lançadas com suporte BranchCache. Estas plataformas são o Windows 7 Enterprise e Ultimate, e o Windows Server 2008 R2. As outras edições do Windows 7 não têm este binário numa localização relevante do sistema e, por conseguinte, são vulneráveis por predefinição. Todas as edições do Windows 7 e Windows Server 2008 R2 podem ser vulneráveis se o DLL BranchCache não existir na localização predefinida.
  
**O que é Windows BranchCache?**    
BranchCache é uma tecnologia de optimização de largura de banda para redes de área ampla (WAN) que é incluída em algumas edições dos sistemas operativos Windows Server 2008 R2 e Windows 7. Para optimizar a largura de banda das WAN, a tecnologia BranchCache copia o conteúdo dos seus servidores de conteúdo do escritório principal e esconde-o em localizações de filiais, permitindo que os computadores cliente localizados nas filiais acedam ao conteúdo localmente e não através da WAN. Para obter mais informações, consulte [Windows Server 2008 R2 e BranchCache](http://www.microsoft.com/windowsserver2008/en/us/branch-cache.aspx).
...
Um intruso poderia convencer um utilizador a abrir um ficheiro legítimo de um tipo aberto por determinadas aplicações do Windows, como .eml e .rss (Windows Live Mail) e .wpost (Microsoft Live Writer) localizado no mesmo directório de rede que um ficheiro de biblioteca de hiperligação dinâmica (DLL) especialmente concebido. Nessa altura, ao abrir o ficheiro legítimo, o BranchCache poderia tentar carregar o ficheiro DLL e executar qualquer código aí contido.
  
Num cenário de ataque com base em correio electrónico, um intruso poderia explorar a vulnerabilidade enviando um anexo de ficheiro .eml, .rss ou .wpost legítimo a um utilizador e convencê-lo a colocar o anexo num directório que tivesse um ficheiro DLL especialmente concebido e abrir o ficheiro legítimo. Então, ao abrir o ficheiro legítimo, o Windows Live Mail ou Microsoft Live Writer podem tentar carregar o ficheiro DLL e executar qualquer código nele contido.
  
Num cenário de rede, um intruso poderia colocar um ficheiro legítimo e um ficheiro DLL especialmente concebido num UNC ou localização WebDAV e convencer o utilizador a abrir o ficheiro legítimo.
...
A actualização resolve esta vulnerabilidade corrigindo o modo como o Windows BranchCache carrega bibliotecas externas.
...
<td style="border:1px solid black;">Consulte o <a href="http://support.microsoft.com/kb/2385678">Artigo 2385678 da Base de Dados de Conhecimento da Microsoft</a></td>
</tr>
<tr class="odd">
...
-   Haifei Li da [Fortinet's FortiGuard Labs](http://www.fortiguard.com/) por fornecer informações sobre a Vulnerabilidade no Carregamento de Biblioteca BranchCache Não Segura (CVE-2010-3966)
