---
TOCTitle: 'MS11-058'
Title: 'Microsoft Security Bulletin MS11-058 - Crítica'
ms:assetid: 'ms11-058'
ms:contentKeyID: 61235039
ms:mtpsurl: 'https://technet.microsoft.com/pt-PT/library/ms11-058(v=Security.10)'
author: SharonSears
...
Microsoft Security Bulletin MS11-058 - Crítica
==============================================

Vulnerabilidades no Servidor de DNS Poderiam Permitir Execução Remota de Código (2562485)
-----------------------------------------------------------------------------------------

Data de publicação: 9 de agosto de 2011 | Updated: 25 de outubro de 2011
...
Esta actualização de segurança está classificada como Crítica para edições de 32 bits e baseadas em x64 do Windows Server 2008 e edições baseadas em x64 do Windows Server 2008 R2, e está classificada como Importante para todas as edições suportadas do Windows Server 2003. Para mais informações, consulte a subsecção **Software Afectado e Software Não Afectado**, nesta secção.

A actualização de segurança resolve as vulnerabilidades modificando a forma como o servidor de DNS processa consultas NAPTR na memória e como inicializa objectos na memória antes de os utilizar. Para mais informações sobre as vulnerabilidades, consulte a subsecção de Perguntas Mais Frequentes quanto à vulnerabilidade específica na secção seguinte, **Informações sobre as Vulnerabilidades**.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------|-------------------------------------|-----------------------------------------------------------|
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=c2d4aa38-9241-465d-8d65-aba73e936d0f)                                                                                           | Negação de Serviço          | Importante                          | [MS09-008](http://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=2db4098a-f4f9-4211-b55d-5d0df1409c43)                                                                               | Negação de Serviço          | Importante                          | [MS09-008](http://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2003 com SP2 para sistemas baseados em Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=1934acfa-5637-4ae9-9e9a-fc7e58930b7a)                                                                | Negação de Serviço          | Importante                          | [MS09-008](http://go.microsoft.com/fwlink/?linkid=139821) |
| [Windows Server 2008 para sistemas de 32 bits Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=ac2d5122-6f9b-46f5-9840-77aa7ff84308)\*                                                                | Execução Remota de Código   | Crítica                             | Nenhum                                                    |
| [Windows Server 2008 para sistemas baseados em x64 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=5f605f6f-3854-403d-878b-637626aef78f)\*                                                           | Execução Remota de Código   | Crítica                             | Nenhum                                                    |
| [Windows Server 2008 R2 para sistemas baseados em x64 e Windows Server 2008 R2 para sistemas baseados em x64 Service Pack 1](http://www.microsoft.com/downloads/details.aspx?familyid=6a86e2cf-4e7d-4012-88c3-6957a3842dd3)\* | Execução Remota de Código   | Crítica                             | Nenhum                                                    |
...
<span></span>
**Por que razão foi revisto este boletim** **a25** **de Outubro de 2011?**
A Microsoft reviu este boletim para anunciar uma alteração da lógica de detecção e para corrigir informações relativas à substituição do boletim para todas as edições suportadas do Windows Server 2003. Não foram efectuadas alterações aos ficheiros de actualização de segurança. Os clientes que já tenham aplicado esta actualização com sucesso não necessitam de efectuar qualquer acção.
...
<th style="border:1px solid black;" >Vulnerabilidade de Consulta NAPTR no DNS - CVE-2011-1966</th>
<th style="border:1px solid black;" >Vulnerabilidade de Corrupção de Memória Não Inicializada no DNS - CVE-2011-1970</th>
...
Vulnerabilidade de Consulta NAPTR no DNS - CVE-2011-1966  
--------------------------------------------------------
  
<span></span>
Existe uma vulnerabilidade de execução remota de código na forma como o servidor de DNS do Windows processa inadequadamente uma cadeia de consulta NAPTR especialmente concebida para o efeito na memória. Um intruso que explorasse esta vulnerabilidade com êxito poderia executar código arbitrário no contexto do sistema. Um intruso poderia então instalar programas; ver, alterar ou eliminar dados; ou ainda criar novas contas com todos os privilégios.
  
Para visualizar esta vulnerabilidade como uma entrada comum na lista de Vulnerabilidades e Exposições Comuns, consulte [CVE-2011-1966](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1966).
  
#### Factores Atenuantes para a Vulnerabilidade de Consulta NAPTR no DNS - CVE-2011-1966
  
A Microsoft não identificou quaisquer factores atenuantes para esta vulnerabilidade.
  
#### Soluções Alternativas para a Vulnerabilidade de Consulta NAPTR no DNS - CVE-2011-1966
...
-   **Desactivar o serviço DNS se não estiver a utilizá-lo**
  
    **Método interactivo:**
  
    1.  Clique em **Iniciar**, em seguida em **Executar**, escreva **regedit** e, depois, clique em **OK**.  
    2.  Expanda **HKEY\_LOCAL\_MACHINE**, expanda **System**, depois expanda **CurrentControlSet** e, finalmente, expanda **Services**.  
    3.  Clique em **DNS**.  
    4.  Clique no menu **Ficheiro** e, depois, seleccione **Exportar**.  
    5.  Na caixa de diálogo Exportar ficheiro de registo, introduza **DNS\_configuration\_backup.reg** e clique em **Guardar**.  
    6.  Faça duplo clique no valor chamado **Start** e, depois, altere o campo **Dados do valor** para **4**.  
    7.  Clique em **OK.**  
    8.  Execute o comando seguinte a partir de uma linha de comandos com privilégios de administrador:  
        `sc`` stop DNS`
  
    **Utilizar a gestão de scripts de implementação:**
  
    1.  Crie uma cópia de segurança das chaves de registo a partir da gestão de scripts de implementação, com o seguinte comando:  
        `regedit`` /e DNS_configuration_backup.reg HKEY_LOCAL_MACHINE\System\``CurrentControlSet``\Services\DNS`  
    2.  Guarde o texto seguinte num ficheiro com a extensão .REG (por exemplo: "Disable\_service\_DNSame.reg").  
        `Editor de registo do Windows versão 5.00`  
        `[HKEY_LOCAL_MACHINE\System\``CurrentControlSet``\Services\DNS]`  
        `"Start"=dword:00000004`  
    3.  Execute o script de registo criado no passo 2 acima na máquina visada utilizando o seguinte comando:  
        `Regedit`` /s Disable_service_DNSame.reg`  
    4.  Execute o comando seguinte a partir de uma linha de comandos com privilégios de administrador:  
        `sc`` stop DNS`
    ...
    **Método interactivo:**
    ...
    1.  Clique em **Iniciar**, em seguida em **Executar**, escreva **regedit** e, depois, clique em **OK**.  
    2.  Clique no menu **Ficheiro** e, depois, seleccione **Importar**.  
    3.  Na janela Importar ficheiro de registo, seleccione **DNS\_configuration\_backup.reg** e clique em **Abrir**.
  
    **Utilizar a gestão de scripts de implementação:**
  
    Reponha o estado original do registo executando o comando seguinte a partir de uma linha de comandos com privilégios de administrador:  
    `Regedit`` /s DNS_configuration_backup.reg`
  
#### Perguntas Mais Frequentes sobre a Vulnerabilidade de Consulta NAPTR no DNS - CVE-2011-1966
  
**Qual é a abrangência desta vulnerabilidade?**  
Esta é uma vulnerabilidade de execução remota de código.
  
**O que provoca a vulnerabilidade?**  
A vulnerabilidade é provocada quando um servidor de DNS não autoritativo processa inadequadamente um registo NAPTR especialmente concebido para o efeito na memória, enquanto solicita recursivamente o registo do servidor de DNS autoritativo.
  
**Qual é o componente afectado por esta vulnerabilidade?**  
O dns.exe é o componente afectado por esta vulnerabilidade.
  
**Como poderia um intruso utilizar a vulnerabilidade?**  
Um intruso que explorasse esta vulnerabilidade com êxito poderia executar código arbitrário no contexto do sistema. Um intruso poderia então instalar programas; ver, alterar ou eliminar dados; ou ainda criar novas contas com todos os privilégios.
  
**Como poderia um intruso explorar a vulnerabilidade?**  
Um intruso remoto não autenticado poderia explorar esta vulnerabilidade registando um domínio, criando um registo de recurso DNS NAPTR e enviando depois uma consulta NAPTR especialmente concebida para o efeito para o servidor de DNS visado.
...
**O que é um registo de recurso DNS NAPTR?**  
Um registo de recurso (RR) DNS do Naming Authority Pointer (NAPTR) permite que o DNS seja utilizado para procurar serviços para uma grande variedade de nomes de recursos, incluindo URIs que não fazem parte da sintaxe do nome de domínio. Para mais informações sobre o NAPTR, consulte [RFC-2915](http://www.ietf.org/rfc/rfc2915.txt).
...
Vulnerabilidade de Corrupção de Memória Não Inicializada no DNS - CVE-2011-1970  
-------------------------------------------------------------------------------
  
<span></span>
Existe uma vulnerabilidade de negação de serviço na forma como o servidor de DNS processa inadequadamente um objecto na memória que não foi inicializado. Um intruso que conseguisse explorar esta vulnerabilidade com êxito poderia fazer com que o serviço do servidor de DNS no sistema visado deixasse de responder.
  
Para visualizar esta vulnerabilidade como uma entrada comum na lista de Vulnerabilidades e Exposições Comuns, consulte [CVE-2011-1970](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1970).
  
#### Factores Atenuantes para a Vulnerabilidade de Corrupção de Memória Não Inicializada no DNS - CVE-2011-1970
  
A Microsoft não identificou quaisquer factores atenuantes para esta vulnerabilidade.
  
#### Soluções Alternativas para a Vulnerabilidade de Corrupção de Memória Não Inicializada no DNS - CVE-2011-1970
  
A Microsoft não identificou quaisquer soluções alternativas para esta vulnerabilidade.
  
#### Perguntas Mais Frequentes sobre a Vulnerabilidade de Corrupção de Memória Não Inicializada no DNS - CVE-2011-1970
  
**Qual é a abrangência desta vulnerabilidade?**  
Esta é uma vulnerabilidade de negação de serviço.
  
**O que provoca a vulnerabilidade?**  
A vulnerabilidade é provocada quando o servidor de DNS processa inadequadamente um objecto não inicializado na memória ao mesmo tempo que procura o registo de recurso de um domínio não existente.
  
**Qual é o componente afectado por esta vulnerabilidade?**  
O dns.exe é o componente afectado por esta vulnerabilidade.
  
**Como poderia um intruso utilizar a vulnerabilidade?**  
Um intruso que conseguisse explorar esta vulnerabilidade com êxito poderia fazer com que o serviço do servidor de DNS deixasse de responder no sistema visado.
  
**Como poderia um intruso explorar a vulnerabilidade?**  
Um intruso remoto não autenticado poderia explorar esta vulnerabilidade enviando uma consulta DNS especialmente concebida para o efeito para o servidor de DNS visado para o registo de recurso de um domínio que não existe.
...
**O que faz a actualização?**  
A actualização resolve a vulnerabilidade modificando a forma como o servidor de DNS inicializa objectos na memória antes de os utilizar.
...
<td style="border:1px solid black;">Utilize a ferramenta <strong>Adicionar ou Remover Programas</strong> no Painel de Controlo ou o utilitário Spuninst.exe que está localizado na pasta %Windir%\$NTUninstallKB2562485$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Informações de Ficheiros</strong></td>
<td style="border:1px solid black;">Consulte o <a href="http://support.microsoft.com/kb/2562485">Artigo 2562485 da Base de Dados de Conhecimento da Microsoft</a></td>
</tr>
<tr class="even">
...
-   Grischa Zengel (Zengel Medizintechnik GmbH), por fornecer informações sobre a Vulnerabilidade de Consulta NAPTR no DNS (CVE-2011-1966)
...
-   V1.1 (21 de Setembro de 2011): Correcção da tabela de Software Afectado para remover o MS11-046 como boletim substituído por esta actualização em todas as edições afectadas do Windows Server 2003. Trata-se apenas de uma alteração de detecção. Não foram efectuadas alterações aos ficheiros de actualização de segurança. Os clientes que já tenham aplicado esta actualização com sucesso não necessitam de efectuar qualquer acção.  
-   V1.2 (25 de Outubro de 2011): Anunciada uma alteração da lógica de detecção e corrigidas as informações relativas à substituição do boletim para algumas configurações afectadas. Não foram efectuadas alterações aos ficheiros de actualização de segurança. Para obter mais informações, consulte as perguntas frequentes sobre a actualização.
